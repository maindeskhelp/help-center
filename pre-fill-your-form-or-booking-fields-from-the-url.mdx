...
All you need to do is use the personalization tag from our example: **{[param.name](http://param.name)}** or **{[param.country](http://param.country)}** anywhere on the page (for example in a text widget) and in the actual page it will display the actual value taken from the URL.
